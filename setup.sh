...
if ask_yes_no "Are you using Claude Code?"; then
    ...
    if grep -q '${PAI_DIR}' "$PAI_DIR/settings.json" 2>/dev/null; then
        print_warning "Fixing hook paths in settings.json..."

        # Replace all instances of ${PAI_DIR} with the actual absolute path
        sed -i "s|\${PAI_DIR}|$PAI_DIR|g" "$PAI_DIR/settings.json"

        print_success "Hook paths fixed in settings.json"
    fi

    # Also fix ${PAI_DIR} in commands/load-dynamic-requirements.md
    if [ -f "$PAI_DIR/commands/load-dynamic-requirements.md" ] && grep -q '${PAI_DIR}' "$PAI_DIR/commands/load-dynamic-requirements.md" 2>/dev/null; then
        print_warning "Fixing paths in load-dynamic-requirements.md..."

        # Replace ${PAI_DIR} with absolute path
        sed -i "s|\${PAI_DIR}|$PAI_DIR|g" "$PAI_DIR/commands/load-dynamic-requirements.md"
    ...
    if command_exists jq; then
        current_pai_dir=$(jq -r '.env.PAI_DIR // empty' "$PAI_DIR/settings.json" 2>/dev/null)
        if [ "$current_pai_dir" != "$PAI_DIR" ]; then
            ...
            jq --arg pai_dir "$PAI_DIR" '.env.PAI_DIR = $pai_dir' "$PAI_DIR/settings.json" > "$PAI_DIR/settings.json.tmp" && \
        ...
        current_da=$(jq -r '.env.DA // empty' "$PAI_DIR/settings.json" 2>/dev/null)
        if [ "$current_da" != "$AI_NAME" ]; then
            print_warning "Updating AI assistant name in settings.json..."

            jq --arg ai_name "$AI_NAME" '.env.DA = $ai_name' "$PAI_DIR/settings.json" > "$PAI_DIR/settings.json.tmp" && \
            mv "$PAI_DIR/settings.json.tmp" "$PAI_DIR/settings.json"
        ...
        current_color=$(jq -r '.env.DA_COLOR // empty' "$PAI_DIR/settings.json" 2>/dev/null)
        if [ "$current_color" != "$AI_COLOR" ]; then
            print_warning "Updating AI assistant color in settings.json..."

            jq --arg ai_color "$AI_COLOR" '.env.DA_COLOR = $ai_color' "$PAI_DIR/settings.json" > "$PAI_DIR/settings.json.tmp" && \
            mv "$PAI_DIR/settings.json.tmp" "$PAI_DIR/settings.json"
    ...
    if [ -f "$PAI_DIR/PAI.md" ]; then
        # Check if PAI.md needs identity assertion added
        if ! grep -q "IMPORTANT: You are" "$PAI_DIR/PAI.md" 2>/dev/null; then
...
IDENTITY_EOF
            ...
            core_identity_line=$(grep -n "^## Core Identity" "$PAI_DIR/PAI.md" | cut -d: -f1)

            if [ -n "$core_identity_line" ]; then
                # Find the next --- after Core Identity
                next_separator_line=$(tail -n +$((core_identity_line + 1)) "$PAI_DIR/PAI.md" | grep -n "^---" | head -1 | cut -d: -f1)
                ...
                {
                    head -n $((core_identity_line - 1)) "$PAI_DIR/PAI.md"
                    cat "$PAI_DIR/.pai_identity_tmp"
                    tail -n +$((next_separator_line + 1)) "$PAI_DIR/PAI.md"
                } > "$PAI_DIR/.pai_md_tmp"

                mv "$PAI_DIR/.pai_md_tmp" "$PAI_DIR/PAI.md"
        ...
        elif grep -q "default: Kai" "$PAI_DIR/PAI.md" 2>/dev/null; then
            # Simple case: just update the name line
            print_warning "Updating AI assistant name in PAI.md..."
            sed -i "s/\*\*Name:\*\* You can customize this (default: Kai)/\*\*Name:\*\* $AI_NAME/" "$PAI_DIR/PAI.md"
    ...
    if [ -f "$PAI_DIR/commands/load-dynamic-requirements.md" ]; then
        ...
        if grep -q "respond like Kai" "$PAI_DIR/commands/load-dynamic-requirements.md" 2>/dev/null; then
            ...
            sed -i "s/respond like Kai having a chat/respond like $AI_NAME having a chat/g" "$PAI_DIR/commands/load-dynamic-requirements.md"
            sed -i "s/You're Kai, their assistant/You're $AI_NAME, their assistant/g" "$PAI_DIR/commands/load-dynamic-requirements.md"
    ...
    if ! grep -q "load-dynamic-requirements.ts" "$PAI_DIR/settings.json" 2>/dev/null; then
        ...
        }' "$PAI_DIR/settings.json" > "$PAI_DIR/settings.json.tmp" && \
        mv "$PAI_DIR/settings.json.tmp" "$PAI_DIR/settings.json"
